...
for x in result:
    ...
    if(retries>=0):
        ...
        print(result2)
        sql3="SELECT contact_details FROM infogate_b2b.user_register WHERE user_id=%s;"  
        mycursor.execute(sql3,(x[1],))
        ...
        for i in res["contacts"]:
            if (i["type"]=="email"):
                response=send_mail(i['address'],message_body)
            elif (i["type"]=="url"):
                response=send_mail(i['address'],message_body)
            elif (i["type"]=="sms"):
                response=send_mail(i['address'],message_body)
